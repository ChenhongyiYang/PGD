...
class DDOD_Distill_Head(DDODHead):
    def __init__(self, **kwargs):
        super(DDOD_Distill_Head, self).__init__(**kwargs)
    ...
    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'iou_preds',
                          'cls_scores_offline', 'bbox_preds_offline', 'iou_preds_offline'))
    def loss(self,
             cls_scores,
             bbox_preds,
             iou_preds,
        ...
        (anchor_list_cls, labels_list_cls, label_weights_list_cls, bbox_targets_list_cls,
         bbox_weights_list_cls, num_total_pos_cls, num_total_neg_cls, kd_value_maps_cls, _) = cls_reg_targets

        num_total_samples = reduce_mean(
            torch.tensor(num_total_pos_cls, dtype=torch.float,
                         device=device)).item()
        num_total_samples = max(num_total_samples, 1.0)

        # get pos samples for each level
        bg_class_ind = self.num_classes
        for ii, each_level_label in enumerate(labels_list_cls):
        ...
        cls_losses_cls, cls_losses_bbox, cls_losses_iou = multi_apply(
            ...
            bbox_targets_list_cls,
            bbox_weights_list_cls,
            reweight_factor_per_level,
        ...
        (anchor_list_reg, labels_list_reg, label_weights_list_reg, bbox_targets_list_reg,
         bbox_weights_list_reg, num_total_pos_reg, num_total_neg_reg, kd_value_maps_reg, kd_back_maps) = cls_reg_targets

        num_total_samples = reduce_mean(
            torch.tensor(num_total_pos_reg, dtype=torch.float,
                         device=device)).item()
        num_total_samples = max(num_total_samples, 1.0)

        # get pos samples for each level
        bg_class_ind = self.num_classes
        for ii, each_level_label in enumerate(labels_list_reg):
            pos_inds = ((each_level_label >= 0)
                        & (each_level_label < bg_class_ind)).nonzero().squeeze(1)
            # num_pos_samples_per_level.append(len(pos_inds))
        ...
        reg_losses_cls, reg_losses_bbox, reg_losses_iou = multi_apply(
            ...
            iou_preds,
            labels_list_reg,
            label_weights_list_reg,
            bbox_targets_list_reg,
            bbox_weights_list_reg,
            reweight_factor_per_level,
            num_total_samples=num_total_samples)
        ...
        if not ret_label_assign:
            ...
            return dict(loss_cls=cls_losses_cls, loss_bbox=reg_losses_bbox, loss_iou=reg_losses_iou), \
                   dict(cls_target=labels_list_reg, reg_target=labels_list_reg,
                        cls_value_maps=kd_value_maps_cls, reg_value_maps=kd_value_maps_reg,
                        back_maps=kd_back_maps)
    ...
    def get_targets(self,
        ...
        (all_anchors, all_labels, all_label_weights, all_bbox_targets,
         all_bbox_weights, pos_inds_list, neg_inds_list, all_kd_value_map, all_kd_back_map) = multi_apply(
        ...
        bbox_weights_list = images_to_levels(all_bbox_weights,
                                             num_level_anchors)
        kd_value_map_list = images_to_levels(all_kd_value_map,
                                             num_level_anchors)
        if not is_cls:
            kd_back_map_list = images_to_levels(all_kd_back_map,
                                             num_level_anchors)
        else:
            kd_back_map_list = None
        return (anchors_list, labels_list, label_weights_list,
                bbox_targets_list, bbox_weights_list, num_total_pos,
                num_total_neg, kd_value_map_list, kd_back_map_list)
    ...
    def _get_target_single(self,
        ...
        assign_result = assigner.assign(anchors, num_level_anchors_inside,
                                            ...
                                            gt_labels)
        if is_cls:
            kd_value_map = self.w_assigner_cls.assign(anchors,
                                          cls_scores_valid, bbox_preds_valid,
                                          gt_bboxes, anchor_levels, gt_bboxes_ignore, gt_labels)
            kd_back_map = None
        else:
            kd_value_map = self.w_assigner_reg.assign(anchors,
                                                      cls_scores_valid, bbox_preds_valid,
                                                      gt_bboxes, anchor_levels, gt_bboxes_ignore, gt_labels)
        ...
        if unmap_outputs:
            ...
            kd_value_map = unmap(kd_value_map, num_total_anchors, inside_flags)
            if not is_cls:
                kd_back_map = unmap(kd_back_map, num_total_anchors, inside_flags)

        return (anchors, labels, label_weights, bbox_targets, bbox_weights,
                pos_inds, neg_inds, kd_value_map, kd_back_map)
