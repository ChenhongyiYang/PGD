...
class GFLDistillHead(GFLHead):
    def __init__(self, **kwargs):
        super(GFLDistillHead, self).__init__(**kwargs)
    ...
    def loss(self,
        ...
        (anchor_list, labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, num_total_pos, num_total_neg, kd_value_maps_cls, kd_value_maps_reg, kd_back_maps) = cls_reg_targets
        ...
        if not ret_label_assign:
            ...
            return dict(loss_cls=losses_cls, loss_bbox=losses_bbox, loss_dfl=losses_dfl), \
                   dict(cls_value_maps=kd_value_maps_cls, reg_value_maps=kd_value_maps_reg, back_maps=kd_back_maps)
    ...
    def get_targets(self,
        ...
        for i in range(num_imgs):
            ...
            for j in range(num_levels):
                ...
                bbox_pred = bbox_preds[j][i].permute(1, 2, 0).reshape(-1, 4 * (self.reg_max + 1))
        ...
        kd_value_map_cls_list = images_to_levels(all_kd_value_cls_map, num_level_anchors)
        kd_value_map_reg_list = images_to_levels(all_kd_value_reg_map, num_level_anchors)
        kd_back_map_list      = images_to_levels(all_kd_back_map, num_level_anchors)
    ...
    def _get_target_single(self,
        ...
        bbox_pred_corners = self.integral(bbox_preds_valid) * strides[0]
        decode_bbox_pred = distance2bbox(anchor_centers, bbox_pred_corners)
        ...
        assign_result = self.assigner.assign(anchors, num_level_anchors_inside,
                                             ...
                                             gt_labels)
        kd_value_cls_map = self.w_assigner_cls.assign(anchors, cls_scores_valid, decode_bbox_pred,
                                                      gt_bboxes, anchor_levels, gt_bboxes_ignore, gt_labels)
        kd_value_reg_map = self.w_assigner_reg.assign(anchors, cls_scores_valid, decode_bbox_pred,
                                                      gt_bboxes, anchor_levels, gt_bboxes_ignore, gt_labels)
